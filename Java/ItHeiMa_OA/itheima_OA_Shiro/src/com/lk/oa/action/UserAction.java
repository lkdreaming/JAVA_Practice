...
public class UserAction extends BaseAction<User> {
	...
	public String add() {
		/**
		 * addUI提交时Ognl表达式并没有让struts去new一个user 所以这里自己new,自己初始化
		 */
		User user = new User();
	...
	public String updateUI() {
		User user = userService.getEntryById(getModel().getUserId());
		//把user放到栈顶用于回显.
		ActionContext.getContext().getValueStack().push(user);
		departmentId = user.getDepartment().getDepartmentId();
		dutyIds = new Long[user.getDuties().size()];
		int index = 0;
		/*
	...
	public String delete() {
		User user = userService.getEntryById(getModel().getUserId());
		BeanUtils.copyProperties(getModel(), user);
		//System.out.println(department.getUserId());
		userService.deleteEntry(user.getUserId());
		return action2action;
	}
