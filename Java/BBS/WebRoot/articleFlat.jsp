...
// 										url += request.getQueryString() == null ? "" : ("?" + request.getQueryString()); 
									...
									<tbody>
										<tr class=<%= classStr %>>
											...
											<%
												if(adminlogined == true) {
											 %>
											<td nowrap="nowrap" width="1%"><a href="javascript:if(confirm('确定要删除吗?')) location='delete.jsp?id=<%=a.getId()%>&isLeaf=<%=a.isLeaf()%>&pid=<%=a.getPid()%>&url=<%=url %>'">删除</a></td>
											<%
												}
											 %>
